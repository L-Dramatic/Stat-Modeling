# 城市空气质量监测预警系统 v2.0 Pro Edition

## 📋 项目概述

本项目是一个基于统计学方法的城市空气质量监测预警系统，采用**由浅入深**的设计思路，完整覆盖统计分析与建模课程的所有核心章节。
...
├── arima_model.py            # ARIMA时间序列模块
└── hmm_model.py              # 隐马尔可夫模型模块
```
...
#### 4. `arima_model.py` - ARIMA时间序列模块

| 功能 | 方法 | 说明 |
|------|------|------|
| 平稳性检验 | `check_stationarity()` | ADF检验（统计量、P值、临界值） |
| 时序分解 | `decompose()` | 趋势 + 季节性 + 残差 |
| 自动选参 | `auto_arima()` | 基于AIC自动选择(p,d,q)参数 |
| 模型拟合 | `fit()` | ARIMA / SARIMA模型 |
| 趋势预测 | `predict()` | 含95%置信区间的未来预测 |

#### 5. `hmm_model.py` - 隐马尔可夫模型模块

| 功能 | 方法 | 说明 |
|------|------|------|
| 状态定义 | `_define_states()` | 基于国标阈值 / 分位数 |
| 模型拟合 | `fit()` | 高斯HMM，观测变量为气象因子 |
| 状态推断 | `predict_states()` | 推断历史隐状态序列 |
| 转移矩阵 | `get_transition_matrix()` | 状态间转移概率 |
| 实时预警 | `predict_current_state()` | 当前状态及概率分布 |
...
| 功能区域 | 组件 | 说明 |
|---------|------|------|
| HMM配置 | 滑块 + 单选 | 状态数、定义方式 |
| 状态结果 | 表格 + 折线图 | 转移矩阵、状态序列 |
| ARIMA配置 | 滑块 | 预测步数 |
| 预测曲线 | Plotly图 | 预测值 + 置信区间 |
...
| **模型解读** | 20% | ⭐⭐⭐⭐⭐ | OLS vs GLM对比、系数解释 |
| **系统创新性** | 10% | ⭐⭐⭐⭐⭐ | HMM实时预警、完整流程 |
| **系统演示** | 20% | ⭐⭐⭐⭐⭐ | 美化UI、三页面完整功能 |
